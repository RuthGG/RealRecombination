...
EUR<-read.table("analysis/2020-05-17_08_tagsnps/EUR_output_plink.ld", header = TRUE)
colnames(EUR)[colnames(EUR)==  "R2"]<-"EUR"

EAS<-read.table("analysis/2020-05-17_08_tagsnps/EAS_output_plink.ld", header = TRUE)
colnames(EAS)[colnames(EAS)==  "R2"]<-"EAS"

SAS<-read.table("analysis/2020-05-17_08_tagsnps/SAS_output_plink.ld", header = TRUE)
colnames(SAS)[colnames(SAS)==  "R2"]<-"SAS"

AFR<-read.table("analysis/2020-05-17_08_tagsnps/AFR_output_plink.ld", header = TRUE)
colnames(AFR)[colnames(AFR)==  "R2"]<-"AFR"

ALL<-read.table("analysis/2020-05-17_08_tagsnps/ALL_output_plink.ld", header = TRUE)
...
# Guardar la tabla: 
write.csv(summary[, c("Inv", "Pop", "good_p", "bad_p", "miss_p", "max")], "report/2020-05-19_tagSNPsCheck_preliminary/tagSNP_summaryTable.csv")
```
