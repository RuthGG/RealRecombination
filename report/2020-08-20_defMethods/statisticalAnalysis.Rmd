...
```{r, echo=FALSE,out.width="45%", fig.cap="Correlació entre mapes de recombinacio calculats amb diferents metodologies. Per comprovar l'efecte del sistema de mesura, s'ha calculat la correlacio entre les taxes de recombinacio del mapa calculat mitjançant el metode nou (eix de les x) i del mapa calculat com el paper original (eix de les y), per mides de finestra de A=500kb, B=100kb, C=50kb, D=20kb.", fig.show='hold',fig.align='center'}



knitr::include_graphics(c("images/correlRecMapQualityTest_500.png","images/correlRecMapQualityTest_100.png","images/correlRecMapQualityTest_50.png","images/correlRecMapQualityTest_20.png"))
...
```{r, echo=FALSE,out.width="45%", fig.cap="Correlacio entre mapes de recombinacio calculats a partir de diferents mides de finestra. Per comprovar l'efecte del sistema de mesura, s'ha calculat la correlacio entre els diferents mapes calculats mitjançant el metode nou. Per cada finestra de 500kb (eix de les x), s'ha calculat la taxa de recombinacio mitjana de les finestres de  A=100kb, B=50kb, C=20kb que hi solapen.", fig.show='hold',fig.align='center'}



knitr::include_graphics(c("images/meansRecMapQualityTest_100.to.500.png","images/meansRecMapQualityTest_50.to.500.png","images/meansRecMapQualityTest_20.to.500.png"))
...
<!-- Taula 1. Filtratge de dades. Cada columna és un grup d'inversions segons la seva mida: G1 fins a 5kb, G2 de 5 a 50kb i G3 més de 50kb. La fila Excluded es aquella on totes les finestres per una inversio en un individu concret tenen exactament la mateixa taxa de recombinacio i per tant no podem diferenciar entre diferents regions. Aixo afectaria nomes a aquells analisis on es comparin diferents tipus de finestra dins de la mateixa inversio. La fila Recombination Rate 0 (RR0) es aquella on la taxa de recombinacio ha estat 0 en totes les finestres associades a la inversio i els seus voltants. El total es la suma de les dues files. -->
...
Per poder comparar entre idividus i inversions, es va dir de normalitzar les taxes de recombinacio en funcio de l'individu i/o el cromosoma, en funcio del que es volgues fer. He fet servir el mapa de recombinació elaborat amb el nostre metode i amb finestres de 500kb com a referència per calcular les mitjanes que necessitem, ja que es la opcio computacionalment menys pesada i com ja hem demostrat abans, la mitjana sortiria la mateixa independentment de la mida de finestra triada per aquesta tasca.

La primera prova de normalització es va fer dividint cada valor per la mitjana en la taxa de recombinació de l'individu, però això no va tenir impacte en què les mitjanes s'igualessin entre individus (Figura 3). 
...
# DATA SUBSET
  ...
  tmp.sample<-melt(tmp.sample[,c("cM.Mb","mean.rec.per.cell", "cM.Mb.chr.norm", "cM.Mb.ind.norm", "cM.Mb.cel.norm", "meanIndRecRate", "ind", "chr_w", "start_w")], id.vars = c("meanIndRecRate" ,"ind", "chr_w", "start_w","mean.rec.per.cell"))
...
![Efecte de diferents normalitzacions sobre les dades. Cada boxplot representa la distribucio de les taxes de recombinació al llarg del genoma per un sol individu. Els individus estan endreçants i colorejats segons el seu nivell de recombinacio global, calculat com la mitjana d'entrecreuaments per cel·lula. Depenent de la mesura que es faci servir, el eix de les y (mostrat aqui en escala logarítmica en base 10) canvia, pero la distribució de les dades es la mateixa.](images/normInvs_alldata.png)


Una teoria de per què això estaria passant és perque la gran quantitat de valors que son zero impideix que s'igualin les mitjanes. Per veure si això seria cert, repeteixo el plot de normalització sense representar els valors que són zero. 

```{r XnormInvs.nozero}

# ALL DATA
# Compare normalizations
  a<-ggplot(recRate.outwins[recRate.outwins$cM.Mb != 0,])+geom_boxplot(aes(x = ind.ordered, y =log(cM.Mb), fill = mean.rec.per.cell))+
  scale_fill_gradient(low = "#1f2896", high = "#d26459")+ggtitle("Original data")
  b<-ggplot(recRate.outwins[recRate.outwins$cM.Mb != 0,])+geom_boxplot(aes(x = ind.ordered, y =log(cM.Mb.ind.norm), fill = mean.rec.per.cell))+
  scale_fill_gradient(low = "#1f2896", high = "#d26459")+ggtitle("Mean individual RR normalization")
  c<-ggplot(recRate.outwins[recRate.outwins$cM.Mb != 0,])+geom_boxplot(aes(x = ind.ordered, y =log(cM.Mb.chr.norm), fill = mean.rec.per.cell))+
  scale_fill_gradient(low = "#1f2896", high = "#d26459")+ggtitle("Mean chromosome RR normalization")
  d<-ggplot(recRate.outwins[recRate.outwins$cM.Mb != 0,])+geom_boxplot(aes(x = ind.ordered, y =log(cM.Mb.cel.norm), fill = mean.rec.per.cell))+
  scale_fill_gradient(low = "#1f2896", high = "#d26459")+ggtitle("Mean RR/cell normalization")

  plotname<-paste0(outdir, "images/normInvs_nozerodata.png")
  png(filename=plotname, width = 1200, height =800, units = "px" )
  ggpubr::ggarrange(a,b,c,d, common.legend =  TRUE, nrow=1 )
  dev.off()
...
Tal com es veu en aquesta imatge, aix sembla que no està passant. 

![Imatge anterior sense representar aquells valors en què la taxa de recombinació és 0](images/normInvs_nozerodata.png)


Una forma de normalitzar les dades es fent el z-score: se li resta la mitjana de l'individu i aquesta resta es divideix per la desviació estàndard de l'individu.

```{r XnormInvs.z-score ,  echo = FALSE, eval = FALSE}
...
####
# MAKE A TEST WITH REAL DATA

mask<-read.table("../../data/use/assembly_hg38/20160622.allChr.mask.bed")

g1<-sample(recMap[recMap$ind == "NC1" & recMap$winsize == 500000 & recMap$group == "new", "cM.Mb"], 5000)
g2<-sample(recMap[recMap$ind == "NC2" & recMap$winsize == 500000 & recMap$group == "new", "cM.Mb"], 5000)
g3<-sample(recMap[recMap$ind == "NC3" & recMap$winsize == 500000 & recMap$group == "new", "cM.Mb"], 5000)
g4<-sample(recMap[recMap$ind == "NC4" & recMap$winsize == 500000 & recMap$group == "new", "cM.Mb"], 5000)
...
sample<-data.frame(g1, g2, g3, g4)
sample<-melt(sample)

ggplot(sample, aes(x = value ,  fill = variable))+geom_histogram(bins = 100)+facet_grid(variable ~ .)+scale_x_log10()


sample  <- sample %>% group_by(variable) %>% mutate(value.scaled = scale(value), mean = mean(value), sd = sd(value))

ggplot(sample, aes(x = value.scaled ,  fill = variable))+geom_histogram(bins = 100)+facet_grid(variable ~ .)#+scale_x_log10()

newstats<-aggregate(value.scaled ~ variable, sample, mean)

ggplot(sample, aes(y = value.scaled ,  x = variable, fill = variable) )+geom_boxplot()+
  ...
  # geom_text(data = newstats, aes(x = variable, y = value.scaled, label = value.scaled), nudge_y = 0.15)+
    geom_text(data = newstats, aes(x = variable, y = value.scaled, label = round(value.scaled, 10)), nudge_y =0.12)
sample$value.scaled<-sample$value
...
norm<-quantile_normalisation(sample)

sample<-melt(data.frame(norm))
#####
# LOG is a requirement
...
testdata<-recMap[recMap$ind == "NC1" & recMap$winsize == 500000 & recMap$group == "new", "cM.Mb"]

# METHOD FROM STACKEXCHANGE

hist(testdata, breaks =100 )

qqnorm(log(testdata+1e-07*1000), ylab = "log (transformed data + ND=RL*1000)")
qqline(log(testdata+1e-07*1000),col="red")

# METHOD OF POWER ADJUSTMENT

  library(rcompanion)

plotNormalHistogram(recMap$meanCO.Mb)

qqnorm(recMap$meanCO.Mb,
       ylab="Sample Quantiles for meanCO per Mb")
qqline(recMap$meanCO.Mb,
       col="red")

T_sqrt<-sqrt(recMap$meanCO.Mb)
plotNormalHistogram(T_sqrt)

T_cub = sign(recMap$meanCO.Mb) * abs(recMap$meanCO.Mb)^(1/3)
plotNormalHistogram(T_cub)
  

T_log = log(recMap$meanCO.Mb+1)
plotNormalHistogram(T_log, )

moments::skewness(recMap$meanCO.Mb)

testdata.small<-sample(testdata, 5000)
T_tuk =
     transformTukey(testdata.small,
                     plotit=FALSE)

testdata.small.transformed<-testdata.small  ^ 0.375

plotNormalHistogram(testdata.small.transformed)

qqnorm(testdata.small.transformed)
qqline(testdata.small.transformed,
       col="red")
#33333333333333333333333333333333
test<-recMap[ recMap$chr == "chr10" & recMap$group == "new" & recMap$winsize==500000, ]

stat_summary(data = test.ind2,aes(x = 2, y = normalized), fun.y=mean, geom="point", color = "red", shape = 3, size = 40) +df_scaled <- test %>% group_by(ind) %>% mutate(cM.Mb.scaled = scale(cM.Mb), mean = mean(cM.Mb), sd = sd(cM.Mb))


test[,zscore:=(cM.Mb-mean(cM.Mb,na.rm=T))/sd(cM.Mb,na.rm=T)]


df_scaled$subst<-df_scaled$cM.Mb-df_scaled$mean

aggregate(subst  ~ ind, df_scaled, mean)
                  
aggregate( cM.Mb.scaled   ~ ind , df_scaled, mean)   
aggregate( cM.Mb.scaled   ~ ind , df_scaled, sd)          


ggplot()+geom_boxplot(data = test.ind1, aes(x = 1, y = cM.Mb))+
  geom_boxplot(data = test.ind2, aes(x = 2, y = cM.Mb))+
    stat_summary(data = test.ind1,aes(x = 1, y = normalized), fun.y=mean, geom="point", color = "red", shape = 3, size = 40) +
  stat_summary(data = test.ind2,aes(x = 2, y = normalized), fun.y=mean, geom="point", color = "red", shape = 3, size = 40) +
  scale_y_log10()
...
![Taxes de recombinacio fora de la inversio en funció del mencanisme de generació. Les dades son la mitjana del resultat de les diferents finestres "out" de cada inversio per un individu determinat. Cada punt representa el percentil que ocupa la taxa de recombinació per una inversio i individu concret en la regio especificada en la distribucio del mapa de recombinacio de l'individu. S'han agrupat les dades pel mecanisme de generació i pel genotip de l'individu per la inversió en qüestio. Cada punt esta colorejat segons la taxa de recombinacio mitjana d'aquell individu.](images/invTypeRel.png)
...
# Valors mitjans descriptius

```{r}
function.InvPlot.means<-function(inversion, recRate.outwins){
 
    test.subset<-aggregate(   cM.Mb.percentile  ~  invID + start_w+end_w + Haplotype +winID,recRate.outwins[recRate.outwins$invID == inversion,], mean)
    
recRate.outwins$counts<-1
test.counts<-aggregate(   counts  ~  invID + start_w+end_w + Haplotype +winID,recRate.outwins[recRate.outwins$invID == inversion,], sum)
test.subset<-merge(test.subset, test.counts)


     a<-      ggplot(test.subset)+geom_rect(aes(xmin=start_w, xmax=end_w, ymin = 0, ymax = cM.Mb.percentile, fill = counts)) + 
...
# for (g in c("G2", "G3")){
  ...
  plotlist<-list()
  recRate.outwins$Genotype_500<-as.character(recRate.outwins$Genotype_500)
   recRate.outwins$Haplotype<-"Homozygous"
recRate.outwins[recRate.outwins$Genotype_500 == "HET","Haplotype"]<-"Heterozygous"
  for(i in unique(recRate.outwins[recRate.outwins$group %in% c("G2", "G3"), "invID"])){
    
   if (length( table(recRate.outwins[recRate.outwins$invID == i,"Haplotype"])) > 1){
   plotlist[[i]]<-function.InvPlot.means(i, recRate.outwins )
   }
  }
  ...
  group1.invs<-recRate.outwins[recRate.outwins$group == "G1",]
  group1.invs$relative.position<-0
  for(i in unique(group1.invs$invID)){
    for (n in unique(group1.invs$ind)){
      tmp<-group1.invs[group1.invs$invID == i & group1.invs$ind ==n ,]
      tmp[order(tmp$start_w),"relative.position"]<-c(-6:6)
      group1.invs[rownames(tmp),"relative.position"]<- tmp$relative.position
    }
  }
  
  inversion.group<-unique(group1.invs[group1.invs$Size > 2000, "invID"])
  
  function.InvPlot.means.groups<-function(inversion.group, recRate.outwins){

    test.subset<-aggregate(   cM.Mb.percentile  ~   relative.position + Haplotype+winID ,recRate.outwins[recRate.outwins$invID %in% inversion.group,], mean)
    
recRate.outwins$counts<-1
test.counts<-aggregate(   counts  ~   relative.position + Haplotype+winID,recRate.outwins[recRate.outwins$invID %in% inversion.group,], sum)
test.subset<-merge(test.subset, test.counts)
test.subset$end<-test.subset$relative.position+1


     a<-      ggplot(test.subset)+geom_rect(aes(xmin=relative.position, xmax=end, ymin = 0, ymax = cM.Mb.percentile, fill = counts)) + 
      facet_grid(. ~ Haplotype, scales = "free")+
      geom_rect(data = test.subset[test.subset$winID=="in",], aes(xmin=relative.position, xmax=end, ymin = 0, ymax = cM.Mb.percentile), fill="goldenrod")+
      geom_rect(data = test.subset[test.subset$winID%in% c("br", "bl"),], aes(xmin=relative.position, xmax=end, ymin = 0, ymax = cM.Mb.percentile), fill="darkgoldenrod")+
       # ggtitle(inversion) +
       theme(strip.text.x = element_text(size = 20), axis.text.y = element_text(size= 20),axis.text.x = element_text(size= 15),title = element_text(size=20))
    return(a)
   ...
   plota<-function.InvPlot.means.groups(inversion.group,group1.invs )
   plota<-plota+ggtitle("Inversions 2kb-5kb")
   
     inversion.group<-unique(group1.invs[group1.invs$Size < 2000 & group1.invs$Size > 1000 , "invID"])
     plotb<-function.InvPlot.means.groups(inversion.group,group1.invs )
    plotb<-plotb+ggtitle("Inversions 1kb-2kb")
    
      inversion.group<-unique(group1.invs[group1.invs$Size < 1000 & group1.invs$Size > 0 , "invID"])
     plotc<-function.InvPlot.means.groups(inversion.group,group1.invs )
    plotc<-plotc+ggtitle("Inversions <1kb")
    
# for (g in c("G2", "G3")){
  
  plotlist<-list(plota, plotb, plotc)

  plotname<-paste0(outdir, paste0("images/invDescription_small",".png"))
  png(filename=plotname, width = 3508/2, height = 2480/4, units = "px" )
  plot<- annotate_figure( ggarrange(plotlist=plotlist, common.legend = TRUE, ncol=3), top = text_grob(paste0("Recombination rate percentiles aggregated by zygosity" ), size = 50) )
  print(plot)
  dev.off()
...
![Efecte dels genotips de les inversions sobre la taxa de recombinacio. Cada punt es la mitjana per a una sola inversio del valor mesurat per tots els individus amb el genotip especificat. El valor mesurat es la diferencia entre la taxa de recombinacio dins la inversio i la mitjana de les finestres a fora (fora-dins). El color del punt indica en base a quants individus s'ha fet aquesta mitjana i per tant ens dona una idea sobre la fiabilitat d'aquesta dada.](images/quantileDifference.png)
